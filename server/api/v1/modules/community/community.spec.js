...
*/
            ...
            .end(() => {
                ...
                client.execute(query,
                    (error, dbresult) => { // eslint-disable-line consistent-return
                        if (error) { logger.debug('First Test', error);
                            return done(error); }
        ...
        request
            ...
            .end((err, result) => {
              if(err) { done(err);}
                ...
                client.execute(query,
                    (error, dbresult) => { // eslint-disable-line consistent-return
                        if (error) { logger.debug('Second Test', error);
                            return done(error); }
                        dbresult.rows[0].domain.should.be.equal(alldata.domain);
                        done();
                    });
    ...
    it('should tell me if the domain is available to register', (done) => {
        request
            ...
            .end((err, result) => {
                ...
                client.execute(query,
                    (error, dbresult) => { // eslint-disable-line consistent-return
                        if (error) { logger.debug('Fourth Test', error);
                            return done(error); }
        ...
        request
            ...
            .end((err, result) => {
                ...
                client.execute(query,
                    (error, dbresult) => { // eslint-disable-line consistent-return
                        if (error) {
                            return logger.debug('Fifth Test', error);
                            done(error); } // eslint-disable-line no-unreachable
        ...
        request
            ...
            .end((err, result) => {
                ...
                client.execute(query,
                    (error, dbresult) => { // eslint-disable-line consistent-return
                        if (error) {
                            return logger.debug('Sixth Test', error);
                            done(error); } // eslint-disable-line no-unreachable
                        dbresult.rows[0].domain.should.be.equal(result.body.domain);
                        dbresult.rows[0].owner.should.be.equal(result.body.owner);
                        done();
    ...
    it('should return an error as not all domains exist when calling for data of multiple domains', (done) => {
        ...
        communityCtrl.getMultipleCommunities(wrongdataarray, (error, result) => {
    ...
    it('should return if delete is requested for a non existing domain', (done) => {
      ...
      communityCtrl.deleteCommunity(domain, (error, result)=> {
        if(error)
       error.should.be.equal('Nothing to Delete');
        client.execute(`select * from communities where domain = '${domain}'`,(err, res) => {
          res.rows.length.should.be.equal(0);
          done();
        })
